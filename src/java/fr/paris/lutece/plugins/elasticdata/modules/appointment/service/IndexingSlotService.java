...
public class IndexingSlotService {	
	        ...
	        ( new Thread( )
	        {
	            ...
	            public void run( )
	            {
	                ...
	                try
		            {
	                    ...
	                    DataSourceService.deleteByQuery( dataSource, AppointmentSlotUtil.buildQueryDateRange( apptFormDTO.getIdForm( ), Timestamp.valueOf( startingDate.atStartOfDay() ).getTime( ) , Timestamp.valueOf( endingDate.atTime(LocalTime.MAX)).getTime( ) ) );
	                    DataSourceService.processIncrementalIndexing( sbuilderLogs, dataSource, collResult );		            
	                }
	                catch( ElasticClientException e )
	                {
    ...
    public static void indexSlot( int nIdSlot, DataSource dataSource )
    {
    	...
    	}else if(!_queueSlotToIndex.contains( nIdSlot )){
     ...
     * @param queueSlotToIndex the queue of slot id
     * @param nIdSlot the id slot
    ...
    private static void indexListSlot(  Queue<Integer> queueSlotToIndex, int nIdSlot,  DataSource dataSource ) throws ElasticClientException {
    	...
    	if(!queueSlotToIndex.contains( nIdSlot )) queueSlotToIndex.add( nIdSlot );
    ...
    {    	
	    ...
	    return new AppointmentSlotDataObject( apptFormdto, slot, AppointmentSlotUtil.INSTANCE_NAME, CategoryHome.findByPrimaryKey( apptFormdto.getIdCategory( ) ) );
	  
    }
    
    /**
     * Build list of AppointmentSlotDataObject
     * @param lisIdSlot the list of id slot
    ...
    {   
    	...
    	for(int nIdSlot: lisIdSlot) {
    		...
    		if( appointmentForm == null) {
    	       ...
    	       collResult.add(
    	                    new AppointmentSlotDataObject( appointmentForm, slot, AppointmentSlotUtil.INSTANCE_NAME,  mapCategory.get(appointmentForm.getIdCategory( )) ) );
    	      }
    	}
